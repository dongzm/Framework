...
{
    public class BaseDaoOra<T, V> : BaseDao<T, V, OracleParameter>
        where T : BaseModel
        where V : List<T>
    {
        ...
        public override T Add(T model)
        {
            ...
            var parameters = new DBParameterProviderOra();
        ...
        public override int DelByCondition(Condition<T> condition)
        {
            ...
            return Convert.ToInt32(DbHelperOra.ExecuteSql(builder.ToString(), parameters.ToArray()));
        }
        ...
        protected override void DeleteBySql(string strWhere, OracleParameter[] parameters)
        {
            ...
            DbHelperOra.ExecuteSql(builder.ToString(), parameters);
        }

        private void FillGetListSqlAndParms(T model, ref StringBuilder strSql, ref DBParameterProviderOra parameters)
        {
            Dictionary<PropertyInfo, CloumnReflectAttribute> propertyCloumnReflectAttr = this._modelAttributes.PropertyCloumnReflectAttr;
        ...
        private void FillSaveSqlAndParms(T model, ref StringBuilder strSql, ref DBParameterProviderOra parameters)
        {
            Dictionary<PropertyInfo, CloumnReflectAttribute> propertyCloumnReflectAttr = this._modelAttributes.PropertyCloumnReflectAttr;
            PropertyInfo[] properties = model.GetType().GetProperties();
            foreach (PropertyInfo info in properties)
        ...
        public override int GetCountByExample(T model)
        {
            ...
            return Convert.ToInt32(DbHelperOra.GetSingle(strSql.ToString(), parameters.ToArray()));
        }

        private DateTime GetCurrentDate()
        {
            return Convert.ToDateTime(DbHelperOra.GetSingle("SELECT sysdate FROM dual"));
        }
        ...
        public override bool IsExists(string id)
        {
            ...
            strSql.Append(" where ");
            this.FillGetModelSqlAndParms(id, ref strSql, ref parameters);
            object single = DbHelperOra.GetSingle(strSql.ToString(), parameters.ToArray());
                ...
                if (propertyCloumnReflectAttr.ContainsKey(info))
                {
                    ...
                    if (attribute.IsPrimaryKey)
                    {
                        if (info.GetValue(model, null) == null)
                        {
                            if (attribute.IsSeq)
                            {
                                var value = DbHelperOra.GetSequences(attribute.SeqName);
