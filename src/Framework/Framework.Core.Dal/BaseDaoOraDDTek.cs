...
{
    public class BaseDaoOraDDTek<T, V> : BaseDao<T, V, OracleParameter>
        where T : BaseModel
        where V : List<T>
    {
        private ModelAttributes _modelAttributes;

        public BaseDaoOraDDTek()
        {
        ...
        public override T Add(T model)
        {
            ...
            var parameters = new DBParameterProviderDDTek();
            string tableName = this._modelAttributes.TableReflect.TableName;
            strSql.AppendFormat("insert into {0}(", tableName);
            var propertyCloumnReflectAttr = this._modelAttributes.PropertyCloumnReflectAttr;
            foreach (var attribute in propertyCloumnReflectAttr.Values)
            {
            ...
            strSql.Remove(strSql.Length - 1, 1);
            strSql.Append(")");
            model = this.FillModelInfoAspect<T>(model);
            this.FillSaveSqlAndParms(model, ref strSql, ref parameters);
            DbHelperOraDDTek.ExecuteSql(strSql.ToString(), parameters.ToArray());
            return model;
        }
        ...
        public override int DelByCondition(Condition<T> condition)
        {
            ...
            DBParameterProviderDDTek parameters = new DBParameterProviderDDTek();
            string tableName = this._modelAttributes.TableReflect.TableName;
            ...
            return Convert.ToInt32(DbHelperOraDDTek.ExecuteSql(builder.ToString(), parameters.ToArray()));
        }
        ...
        public override int DelByCondition(List<Condition<T>> conditionList)
        {
            ...
            return Convert.ToInt32(DbHelperOraDDTek.GetSingle(builder.ToString(), parameters.ToArray()));
        }
        ...
        protected override void DeleteBySql(string strWhere, OracleParameter[] parameters)
        {
            StringBuilder builder = new StringBuilder();
            string tableName = this._modelAttributes.TableReflect.TableName;
            ...
            DbHelperOraDDTek.ExecuteSql(builder.ToString(), parameters);
        }

        private void FillGetListSqlAndParms(T model, ref StringBuilder strSql, ref DBParameterProviderDDTek parameters)
        {
            Dictionary<PropertyInfo, CloumnReflectAttribute> propertyCloumnReflectAttr = this._modelAttributes.PropertyCloumnReflectAttr;
        ...
        private void FillGetModelSqlAndParms(string id, ref StringBuilder strSql, ref DBParameterProviderDDTek parameters)
        {
            Dictionary<PropertyInfo, CloumnReflectAttribute> propertyCloumnReflectAttr = this._modelAttributes.PropertyCloumnReflectAttr;
        ...
        private W FillModelInfoAspect<W>(W model)
        {
            Dictionary<PropertyInfo, CloumnReflectAttribute> propertyCloumnReflectAttr = this._modelAttributes.PropertyCloumnReflectAttr;
            foreach (CloumnReflectAttribute attribute in propertyCloumnReflectAttr.Values)
            {
        ...
        private void FillSaveSqlAndParms(T model, ref StringBuilder strSql, ref DBParameterProviderDDTek parameters)
        {
            Dictionary<PropertyInfo, CloumnReflectAttribute> propertyCloumnReflectAttr = this._modelAttributes.PropertyCloumnReflectAttr;
            PropertyInfo[] properties = model.GetType().GetProperties();
            foreach (PropertyInfo info in properties)
        ...
        public override int GetCountByExample(T model)
        {
            ...
            return Convert.ToInt32(DbHelperOraDDTek.GetSingle(strSql.ToString(), parameters.ToArray()));
        }

        private DateTime GetCurrentDate()
        {
            return Convert.ToDateTime(DbHelperOraDDTek.GetSingle("SELECT sysdate FROM dual"));
        }
        ...
        protected override DataSet GetList(string strWhere, OracleParameter[] parameters, PagingInfo pagingInfo, string orderby)
        {
            ...
            return DbQueryHelperOraDDTek.Query(builder.ToString(), parameters, pagingInfo, orderby);
        }
        ...
        private string GetSqlAndParmsByCondition(Condition<T> condition, ref DBParameterProviderDDTek parameters)
        {
            object obj2;
            string cloumnName;
            ArrayList list;
        ...
        public override bool IsExists(string id)
        {
            ...
            strSql.Append(" where ");
            this.FillGetModelSqlAndParms(id, ref strSql, ref parameters);
            object single = DbHelperOraDDTek.GetSingle(strSql.ToString(), parameters.ToArray());
                ...
                if (propertyCloumnReflectAttr.ContainsKey(info))
                {
                    ...
                    if (attribute.IsPrimaryKey)
                    {
                        if (info.GetValue(model, null) == null)
                        {
                            if (attribute.IsSeq)
                            {
                                var value = DbHelperOraDDTek.GetSequences(attribute.SeqName);
